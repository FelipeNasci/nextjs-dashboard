...
import { authorize, signIn } from '@/auth';
...
function invoiceValidation(formData: FormData) {
  return CreateInvoice.safeParse({
...
export async function createInvoice(_: State, formData: FormData) {
  const { error, success, data } = invoiceValidation(formData);
